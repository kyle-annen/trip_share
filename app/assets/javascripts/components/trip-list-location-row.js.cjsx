@TripListLocationRow = React.createClass
  render: ->
    <tbody className="table table-hover table-striped">
	    ...
	    <tr className={'aria-hidden location-menu-bar hidden location-button-row ' + 'location-row-' + this.props.tripLocation.id} id={'location-menu-bar-' + this.props.tripLocation.id}>
	      <td colSpan="3" >
	      ...
	      <td>
	        ...
	        <span> 	</span><span className="fa fa-times" aria-hidden="true"></span></button>
				...
				{ this.props.tripLocation.restaurants.map((restaurant) => <TripListLocationRowRestaurantRow key={restaurant.id} restaurant={restaurant} rowclass={'location-res-bar-' + this.props.tripLocation.id} />)}
